...
export default async function RelatorioPublico({ params }: { params: Promise<{ token: string }> }) {
  ...
  const rotuloObjetivo = (c: string) => OBJETIVOS.find(([k]) => k === c)?.[1] ?? c;
  ...
  return (
    <main className="mx-auto max-w-3xl px-5 py-10">
      ...
      <header className="rounded-2xl bg-ink px-8 py-9 text-cream">
        ...
        <p className="rotulo !text-gold">Raio-X 360 · por Nº 5</p>
        <h1 className="mt-1.5 font-display text-4xl font-extrabold tracking-tight">
          {cliente?.nome_marca ?? "Relatório"}
        </h1>
      ...
      {Object.values(atual).some(Boolean) && (
        <section className="mt-8">
          <p className="rotulo">o que tens hoje</p>
          <h2 className="mb-3 font-display text-2xl font-extrabold">O ponto de partida</h2>
      ...
      {resultados.length > 0 && (
        <section className="mt-8">
          <p className="rotulo">o site, por dentro</p>
          <h2 className="mb-3 font-display text-2xl font-extrabold">O que encontrámos</h2>
      ...
      {redes.length > 0 && (
        <section className="mt-8">
          <p className="rotulo">as redes</p>
          <h2 className="mb-3 font-display text-2xl font-extrabold">Como está cada canal</h2>
      ...
      {(objetivos.selecionados?.length || objetivos.texto_livre) && (
        <section className="mt-8">
          <p className="rotulo">o que queres</p>
          <h2 className="mb-3 font-display text-2xl font-extrabold">Onde queres chegar</h2>
          <div className="rounded-xl border border-line bg-white p-5">
      ...
      {recs.length > 0 && (
        <section className="mt-8">
          <p className="rotulo">onde entramos</p>
          <h2 className="mb-3 font-display text-2xl font-extrabold">Onde a Nº 5 pode ajudar</h2>
      ...
      <section className="mt-8 rounded-2xl bg-ink px-8 py-9 text-center text-cream">
        <p className="font-display text-2xl font-extrabold">Damos cá cinco? 🖐️</p>
        <p className="mx-auto mt-2 max-w-md text-sm text-soft">
          A nossa garantia de honestidade: se não encontrarmos pelo menos 3 coisas concretas para
          melhorar, dizemos-te na hora — e não avançamos.
        </p>
      ...
      <footer className="mt-6 text-center text-[11px] text-soft">
        Raio-X preliminar, a partir da informação pública disponível. As avaliações das redes
        refletem a análise da equipa Nº 5. Não constitui garantia de resultados. · Nº 5, marca
        operada por Os Caetanos, Lda · NIF 504428918
      </footer>
    </main>
  );
}
